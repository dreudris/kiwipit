...
> **A freshly created Worker serves a `Hello world` placeholder until the first Git build lands.** When you create a Worker via Connect-to-Git, Cloudflare provisions it immediately with a stub (`return new Response("Hello world")`) and the DNS + cert come up right away — so the subdomain looks "live" but every route (including `/api/*`) returns `Hello world`. The real `worker.js` + static assets only appear after Workers Builds runs `wrangler deploy`. Push a commit to `main` to trigger that build, then confirm with the smoke tests below — a Worker whose `modified_on` equals its `created_on` has never had a successful build deploy.

> **Renaming the Worker requires dashboard work, not just a config edit.** Workers Builds is bound to a specific Worker by its dashboard connection — editing `name:` in `wrangler.jsonc` alone has no effect; the deploy keeps landing on the original Worker. To actually rename: in the dashboard, disconnect Workers Builds from the existing Worker (Settings → Builds), then **Workers & Pages → Create → Connect to Git** with the new name, which provisions a fresh Worker. Move custom domains (e.g. `kiwipit.dreudris.com`) separately via Settings → Domains & Routes — they don't follow the rename. A custom domain is a unique binding: exactly one Worker can own a given hostname at a time (like a TCP port). So always *release the domain from the old Worker first, then add it to the new one* — doing it in the other order makes the new Worker's deploy fail with a domain conflict. Delete (or leave dormant) the old Worker once verified.
