...
description: "On Pitfolio, describe the diff and stop before committing — let the user authorize commit/push explicitly"
metadata: 
  node_type: memory
  type: feedback
...
After implementing a change on Pitfolio, describe what changed in a short summary and stop. Do not auto-commit, even when CLAUDE.md's phase workflow lists "implement → commit → push" as a single sequence. The user authorizes with an explicit "commit it" / "push it" / "commit and push" each time.
