...
The fix is to put a tiny middle-man on Cloudflare's servers. The browser asks `https://kiwipit.com/api/solana` (same domain as our site, so no browser blocks), and `worker.js` forwards that request to Solana's RPC and pipes the answer back. From the browser's point of view, it's just talking to itself.
